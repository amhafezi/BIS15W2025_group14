...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      width = 5,  # Controls the width of the question panel
      div(class = "panel-box",
          h3("Tell us about your ideal dog!"),
          selectInput("size", "Preferred Dog Size:", choices = c("Not Important", unique(dog_data$size))),
          selectInput("coat", "Preferred Coat Type:", choices = c("Not Important", unique(dog_data$coat))),
          selectInput("coat_color", "What coat color do you prefer?", choices = c("Not Important", unique(dog_data$color))),
          selectInput("sex", "Preferred Sex:", choices = c("Not Important", unique(dog_data$sex))),
          sliderInput("age", "Preferred Age Range:",
                      ...
                      step = 1),
          selectInput("good_with_kids", "Good with children?", choices = c("Not Important", "yes")),
          selectInput("good_with_males", "Good with male dogs?", choices = c("Not Important", "yes")),
          selectInput("good_with_females", "Good with female dogs?", choices = c("Not Important", "yes")),
          selectInput("good_with_cats", "Good with cats?", choices = c("Not Important", "yes")),
          selectInput("house_trained", "House Trained?", choices = c("Not Important", "yes")),
          selectInput("neutered", "Neutered?", choices = c("Not Important", "yes", "no")),
          br(),
    ...
    mainPanel(
      width = 7,  # Controls the width of the results panel
      div(class = "panel-box",
...
server <- function(input, output, session) {
  ...
  filtered_data <- eventReactive(input$find, {
    df <- dog_data
    if (input$size != "Not Important") df <- df %>% filter(size == input$size)
    if (input$coat != "Not Important") df <- df %>% filter(coat == input$coat)
    if (input$coat_color != "Not Important") df <- df %>% filter(color == input$coat_color)
    if (input$sex != "Not Important") df <- df %>% filter(sex == input$sex) 
    df <- df %>% filter(age >= input$age[1] & age <= input$age[2])
    if (input$good_with_kids == "yes") df <- df %>% filter(likes_children == "yes")
    if (input$good_with_males == "yes") df <- df %>% filter(get_along_males == "yes")
    if (input$good_with_females == "yes") df <- df %>% filter(get_along_females == "yes")
    if (input$good_with_cats == "yes") df <- df %>% filter(get_along_cats == "yes")
    if (input$house_trained == "yes") df <- df %>% filter(housebroken == "yes")
    if (input$neutered != "Not Important") df <- df %>% filter(neutered == input$neutered)
    df
  })
